public class TichTieuScreen : PaintListScreen
{
	...
	public static TichTieuScreen instance;
	...
	public TichTieuScreen(mVector vec, int ruby)
		: base(0, vec, T.tichTieuRuby, 200, 180)
	{
		rubyDaNap = ruby;
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 7:
		{
			...
			TichTieuInfo.instance = new TichTieuInfo(type, infoMemList);
			TichTieuInfo.instance.Show(GameCanvas.gameScr);
			break;
		}
		case 8:
		{
			idSelect = subIndex;
			InfoMemList infoMemList = (InfoMemList)vecPlayer.elementAt(idSelect);
			GlobalService.gI().Send_Nhan_TichTieuScr(1, (sbyte)infoMemList.id);
			break;
		}
		case 9:
		{
	...
	public override void paintInfo(mGraphics g, InfoMemList mem, int xpaint, int ypaint, int i, int wsub)
	{
		if (mem != null)
		{
			...
			AvMain.Font3dColor(g, T.tichTieuRuby + " " + Interface_Game.ConvertNumToStr(mem.quaNapThe.cost) + " " + T.ruby, xpaint + wsub / 2, ypaint + 2, 2, 5);
			if (GameCanvas.isTouch)
			{
	...
	public override void paintBg(mGraphics g)
	{
		...
		AvMain.FontBorderColor(g, T.ruby + " :", xSub + wSub / 2 - 2, ySub + 37, 1, 1, 7);
