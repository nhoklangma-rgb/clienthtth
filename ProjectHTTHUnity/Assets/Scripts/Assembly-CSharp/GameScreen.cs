...
{
	public static int modSpeedMode = 1; // x1..x4
	public static bool modGomQuai = false;
	public static bool modTanSat = false;
	public static bool modAutoRevive = false;

	private const string MOD_SETTINGS_KEY = "MOD_AUTO_SETTINGS_V2";

	public static void ApplyModSpeedMode(int mode)
	{
		if (mode < 1) mode = 1;
		if (mode > 4) mode = 4;
		modSpeedMode = mode;
		GameCanvas.percent = (mode - 1) * 100;
	...
	public static void SaveModSettings()
	{
		try
		{
			...
			dos.writeByte(modTanSat ? 1 : 0);
			dos.writeByte(modGomQuai ? 1 : 0);
			dos.writeByte(modAutoRevive ? 1 : 0);
			dos.writeByte(modSpeedMode);
			GameMidlet.saveRMS(Environment.MachineName + MOD_SETTINGS_KEY, bos.toByteArray());
			dos.close();
		}
		catch (Exception)
		{
	...
	public static void LoadModSettings()
	{
		try
		{
			sbyte[] data = GameMidlet.loadRMS(Environment.MachineName + MOD_SETTINGS_KEY);
			if (data == null || data.Length < 4) return;
			ByteArrayInputStream bis = new ByteArrayInputStream(data);
			DataInputStream dis = new DataInputStream(bis);
			modTanSat = dis.readByte() == 1;
			modGomQuai = dis.readByte() == 1;
			modAutoRevive = dis.readByte() == 1;
			ApplyModSpeedMode(dis.readByte());
			dis.close();
		}
		catch (Exception)
		{
	...
	public override void Show()
	{
		...
		setNewEventCLan();
		Player.isAutoRevice = modAutoRevive;
		Player.AutoRevice = (sbyte)(modAutoRevive ? 1 : 0);
		if (player != null)
		{
			player.isAutoFireNew108 = modTanSat;
			player.isGomQuai112 = modGomQuai;
		}
	}
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 103:
		{
			iCommand iCommand2 = new iCommand("x1", 104, this);
			iCommand iCommand3 = new iCommand("x2", 105, this);
			iCommand iCommand4 = new iCommand("x3", 106, this);
			iCommand iCommand5 = new iCommand("x4", 120, this);
			if (modSpeedMode == 1)
			{
				...
				iCommand5.isDisplay = false;
				iCommand5.isSaved = false;
				iCommand2.isDisplay = true;
				iCommand2.isSaved = true;
			}
			else if (modSpeedMode == 2)
			{
			...
			else if (modSpeedMode == 4)
			{
				iCommand5.isDisplay = true;
				iCommand5.isSaved = true;
				iCommand2.isDisplay = false;
				iCommand2.isSaved = false;
				iCommand3.isDisplay = false;
				iCommand3.isSaved = false;
			...
			mVector2.addElement(iCommand5);
			GameCanvas.Start_Normal_DiaLog(T.chooseSpeedupTitle, mVector2, isCmdClose: true);
			break;
		}
		case 104:
			ApplyModSpeedMode(1);
			saveSpeedUpRMS(GameCanvas.percent);
			SaveModSettings();
			GameCanvas.end_Dialog();
			GameCanvas.Start_Normal_Only_CmdClose_DiaLog("Speed x1");
			break;
		case 105:
			ApplyModSpeedMode(2);
			saveSpeedUpRMS(GameCanvas.percent);
