public class MsgInfoMemSudo : MsgDialog
{
	...
	public MsgInfoMemSudo(InfoMemList mem)
	{
		...
		{
			mSystem.outz("Player.ChucInSudo " + Player.ChucInSudo + " mem.chucInSudo " + mem.chucInSudo);
			if (Player.ChucInSudo != 1 && Player.ChucInSudo == 2)
			{
				cmdLeave = new iCommand(T.leaveSudo, 11, this);
		...
		{
			cmdList.addElement(cmdClose);
			cmdClose.setPos(MotherCanvas.hw + (wDia - 50) / 2, yDia - GameCanvas.hCommand / 2, MainTab.fraCloseTab, "");
		}
		backCMD = cmdClose;
	}
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 3:
			GlobalService.gI().Sudo_CMD(16, mem.name, (short)mem.id, 0);
			GameCanvas.end_Dialog();
			break;
		case 4:
		...
		case 9:
			GlobalService.gI().Sudo_CMD(17, "", (short)mem.id, 0);
			GameCanvas.end_Dialog();
			break;
		case 10:
			GameCanvas.Start_Normal_DiaLog(T.banmuon + mem.name + " " + T.roiSudo, new iCommand(T.kickClan, 3, this), isCmdClose: true);
			break;
		case 11:
			GameCanvas.Start_Normal_DiaLog(T.banmuon + T.roiSudo, new iCommand(T.leaveSudo, 9, this), isCmdClose: true);
			break;
		case 12:
			GlobalService.gI().Clan_CMD(15, "", 0, 0);
			GameCanvas.end_Dialog();
			break;
		case 13:
	...
	public override void paint(mGraphics g)
	{
		...
		mFont.tahoma_7b_black.drawString(g, mem.getCaptionSudo(mem.chucInSudo), num2 + miniItem * 2 + 16, num + GameCanvas.hText - 1, 0);
		mFont.tahoma_7b_black.drawString(g, T.capdo + ": " + mem.Lv, num2, num + GameCanvas.hText * 2 - 1, 0);
		mFont.tahoma_7b_black.drawString(g, T.diemSudo + ": " + mem.expSudo, num2, num + GameCanvas.hText * 3 - 1, 0);
		if (cmdList != null)
		{
	...
	public override void updatekey()
	{
		...
		if (GameCanvas.isTouchNoOrPC())
		{
			if (idCommand >= 0 && idCommand < cmdList.size() && ((iCommand)cmdList.elementAt(idCommand)).caption.Length == 0)
			{
				idCommand = 0;
			}
			for (int i = 0; i < cmdList.size(); i++)
