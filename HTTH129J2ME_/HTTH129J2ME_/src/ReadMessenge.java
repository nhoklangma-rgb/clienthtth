   ...
   public final void ChangeMap(Message m) {
      try {
         ...
         if (setMapSea(num = m.reader().readShort())) {
            // Fixed: Load sea map immediately instead of just storing message
      ...
      if (LoadMap.mSea != null) {
         for(int i = 0; i < LoadMap.mSea.length; ++i) {
            if (LoadMap.mSea[i][1] == id && LoadMap.mSea[i][0] == GameCanvas.loadmap.idLastMap) {
               // Fixed: Add buff for sea map transition but don't return true to avoid special handling
               GameScreen.player.addEffBuff((byte)(LoadMap.mSea[i][2] - 1), LoadMap.mSea[i][3], LoadMap.mSea[i][4]);
               return false; // Always return false to load map normally
            }
         }
      }
      ...
      try {
         ...
         if (b == 3) {
            ...
            for(int i = 0; i < id; ++i) {
               ...
               // Create PetData for PetManager
               PetData petData = new PetData(id2, name, idIcon, isUse, upgrade);
               PetManager.addPet(petData);
            ...
            // Pet items are now handled by server only
