...
public final class Player extends MainPlayer {
   ...
   public Player() {
      ...
      for(int var1 = 0; var1 < hotkeyPlayer.length; ++var1) {
         hotkeyPlayer[var1] = new Hotkey[10];
   ...
   public void updateGomQuai() {
        AThMadaraFunc.updateAutoGom(this, isGomQuai112);
    }
   ...
   public final void update() {
      ...
      if (super.Action != 4 && super.Hp > 0 && !isGhost) {
         ...
         if (AutoFireCur > 0 && super.Action != 2 && Interface_Game.isAutoFireInterface) {
            ...
            if (typeAutoBuff == 1 && MsgAutoFire.value != null && GameCanvas.gameTick % 2 == 0) {
               pp2 = this;
...
//         Ageticon.run();//kéo icon ok
//         Ageteff.run();//kéo effect ok
//         Agetpart.run();//kéo part ok 
//         Agetmap.run(0, 32767, 1);//kéo map no
        //Ageteffauto.run();// ok
        //Agetpet.run(new Message((byte)-80));//no
   ...
   public final void setActionHotKey(int index) {
      if (index != 999) {
         if (index < 0 || index >= hotkeyPlayer[currentTab].length) {
            return;
         }
         GameCanvas.clearAll();
         if (isGhost) {
   ...
   public final void beginPlayerFire(int var1) {
      if (var1 < 0 || var1 >= hotkeyPlayer[currentTab].length) {
         return;
      }
      ...
      if (var1 != null && !var1.returnAction() && var1.Action != 4) {
         if (GameCanvas.loadmap.mapLang()) {
            ...
            if (var1.Lv < 0) {
               ...
               for(int var2 = 0; var2 < mSatnhan.length; ++var2) {
                  if (var1.ID == mSatnhan[var2]) {
                     return true;
                  }
               }
   ...
   public final void setAutoFire(boolean isAutoNew) {
      if (GameCanvas.timeNow - this.timeFristSkill > 100L && super.skillCurrent == null) {
         if (isBack) {
